		...
		if(SPIFFS.begin()){
			if(SPIFFS.exists("/config_lamps.json")){
				...
				if (config_file) {
					...
					conf["osPodjazd"]["btnPin"] = 32;
					conf["osOglSciezka"]["btnPin"] = 35;
					...
					conf["osAltana"]["lighteningTime"] = 0;
					conf["osOgrod"]["lighteningTime"] = 0;
					conf["osPodjazd"]["lighteningTime"] = 25;
					conf["osOglSciezka"]["lighteningTime"] = 0;
